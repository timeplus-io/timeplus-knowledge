...
- **Two-tier authorization.** A built-in `admin` role gets everything, including user and role management; custom roles are chat-only.
- **Role-scoped chat.** A role lists the exact `name@ref` corpus entries its members may query; a non-admin's chat answers are transparently restricted to that scope (admin, MCP, and CLI are unrestricted). Isolation is enforced server-side across every graph tool path.
- **Admin console.** A Users/Roles tab manages accounts, role assignments, password resets, and per-role entry-key access; last-admin lockout is prevented.
